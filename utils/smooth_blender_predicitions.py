...
# 
# MartinHasal - removed some minor bugs and replaced deprecated functions
#               added function for cleaning of small unnconected elements
...
def get_dummy_img(xy_size=128, nb_channels=3):
    ...
    lin = np.expand_dims(
        ...
        nb_channels)
    """
    xv, yv = np.meshgrid(np.linspace(0, 1, xy_size), np.linspace(1, 0, xy_size))
    lin = (xv*yv)
    lin = np.dstack((lin,lin,lin))
    x = x * lin
...
def round_predictions(prd, nb_channels_out, thresholds):
    ...
    # works, but it is not necessary, it just reproduce original githu

...
def buildModel(input_shape, nclasses: int = 2, encoder_type: str = 'MobileNetV2', trainable_encoder=True):

    unet = UnetMobileNetV2(input_shape, nclasses=nclasses, encoder_type=encoder_type, trainable_encoder=trainable_encoder)
    nn_unet = unet.model
    nn_unet.summary()
...
if __name__ == '__main__':
    ...
    NSAMPLES = 4
    predictDataset(ds_test, nsamples_to_plot=NSAMPLES, nn_model=nn_unet_MobileNetV2)
    ...
    plotPredictedImg(path_tst_img, path_tst_label, predictImg, nn_model=nn_unet_MobileNetV2)
    plotPredictedImgSlicer(path_tst_img, path_tst_label, predictImg, nn_model=nn_unet_MobileNetV2)

    plotColorizedVessels(path_tst_img, predictImg, nn_model=nn_unet_MobileNetV2)
    plotHistogramImgSlicer(path_tst_img, path_tst_label, predictImg, nn_model=nn_unet_MobileNetV2)
    ...
    if OUTPUT_MODEL_PATH is not None:
        with elapsed_timer('Saving model (MobileNetV2)'):
            save_model(nn_unet_MobileNetV2, OUTPUT_MODEL_PATH, OUTPUT_MODEL_NAME)
